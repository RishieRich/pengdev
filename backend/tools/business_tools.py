...
def _missing_data_message(data: dict) -> str:
    missing = ", ".join(data.get("missingFiles", [])) or "required workbook files"
    return f"Input data missing. Please add {missing} in the input folder before asking business questions."
...
def get_business_summary() -> str:
    ...
    if not data.get("available"):
        return _missing_data_message(data)
    h = data["headline"]
    return (
...
def get_top_customers() -> str:
    ...
    if not data.get("available"):
        return _missing_data_message(data)
    for i, c in enumerate(data["topCustomers"], 1):
        lines.append(
...
def get_top_vendors() -> str:
    ...
    if not data.get("available"):
        return _missing_data_message(data)
    for i, v in enumerate(data["topVendors"], 1):
        lines.append(
...
def get_top_products() -> str:
    ...
    if not data.get("available"):
        return _missing_data_message(data)
    for i, p in enumerate(data["topProducts"], 1):
...
def get_top_materials() -> str:
    ...
    if not data.get("available"):
        return _missing_data_message(data)
    for i, m in enumerate(data["topMaterials"], 1):
